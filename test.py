...
def extract_next_links(url, resp):
    ...
    for link in soup.find_all("a"):
        ...
        if new_url and is_valid(new_url):
            ...
            abs_link, frag = urldefrag(abs_link)
            if abs_link and is_valid(abs_link) and not frag:
                stats.update_subdomain_count(abs_link)
...
def is_valid(url):
    ...
    try:
        ...
        if stats.is_trap(parsed):
            print("TRAP FOUND", url + '\n')
            stats.update_traps(parsed.netloc + parsed.path)
            return False
